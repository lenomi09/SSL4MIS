...
def get_current_consistency_weight(epoch):
    ...
    # Increased rampup schedule for better convergence
    return args.consistency * ramps.sigmoid_rampup(epoch, args.consistency_rampup / 2)
...
def train(args, snapshot_path):
    ...
    # Weighted CE loss to handle class imbalance
    class_weights = torch.tensor(
        [0.1, 0.3, 0.3, 0.3]
    ).cuda()  # Adjust weights based on class frequency
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # Increased weight for dice loss (0.4) vs ce loss (0.6) for better dice optimization
            supervised_loss = (
            ...
            # Improved learning rate schedule using cosine annealing instead of linear
            lr_ = base_lr * (1.0 + math.cos(math.pi * iter_num / max_iterations)) / 2
